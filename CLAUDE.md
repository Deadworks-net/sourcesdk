...
This fork (`Deadworks-net/sourcesdk`) targets **Deadlock**, not CS2. Deadlock's engine is a snapshot between CS2 beta and CS2 release, so neither upstream's `CS2_BETA` nor its release layouts match it exactly. Upstream `6119e91b` ("Remove CS2_BETA defines") is where most divergence originates.

Every intentional divergence is marked with a `// Deadlock:` comment at the site. When merging `Wend4r/sourcesdk`, keep these, and run the deadworks build + a map load before committing the submodule bump:

| File | Keep |
| --- | --- |
| `public/entity2/entityinstance.h` | `GetSerializerClassInfo()` at slot 0 (upstream has it; `NetworkStateChanged` must be slot 28) |
| `public/entity2/entitysystem.h` | `CEventQueue::m_iListCount`; `offsetof( CGameEntitySystem, m_entityListeners ) == 0x20D0` |
| `public/schemasystem/schematypes.h` | no `SchemaClassInfoData_t::m_pszCPPName`; `offsetof( m_pFields ) == 0x28` |
...
```text
dumpbin /EXPORTS tier0.dll  ->  lib/win64/tier0.def  (names only, no @ordinal)
lib /DEF:lib/win64/tier0.def /MACHINE:X64 /OUT:lib/win64/tier0.lib
```

Before trusting a header layout against Deadlock, compile a small `offsetof` probe rather than reasoning from the diff; the MCP servers here are named `ida-pro-mcp*`, not `ida-mcp-*`.
